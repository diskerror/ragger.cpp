...
// Import (do_import) moved to src/import.cpp / include/import.h.
// User-command helpers (read_password, provision_user, and the
// useradd/usermod/userdel/add-self/passwd verbs) moved to
// src/user_commands.cpp / include/user_commands.h.
...
int main(int argc, char **argv) {
    ...
    try {
        if (command == "start" || command == "stop" ||
            ...
            return ragger::cmd_useradd(opts, cfg.resolved_db_path());
        }
        else if (command == "usermod") {
            return ragger::cmd_usermod(opts, cfg.resolved_db_path());
        }
        else if (command == "userdel") {
            return ragger::cmd_userdel(opts, cfg.resolved_db_path());
        }
        else if (command == "add-self") {
            return ragger::cmd_add_self(cfg.resolved_db_path());
        }
        else if (command == "passwd") {
            return ragger::cmd_passwd(opts, cfg.resolved_db_path());
        }
